...
<body>
    ...
    <style>
        ...
        .google-header {
            ...
            min-height: 64px; /* Standard Google header height */
            padding: 8px 0;
        }
    ...
    <header class="navbar fixed-top p-0 google-header no-print flex-wrap flex-md-nowrap align-items-center">
        ...
        <div class="d-flex nav-side-left w-100 w-md-auto align-items-center justify-content-between px-2 px-md-0 order-1">
            <div class="d-flex align-items-center">
                <!-- Mobile Toggle -->
                <button class="navbar-toggler d-lg-none collapsed border-0 p-1 me-2 ms-2" type="button" data-bs-toggle="collapse" data-bs-target="#sidebarMenu" aria-controls="sidebarMenu" aria-expanded="false" aria-label="Toggle navigation">
                    <span class="material-symbols-outlined" style="font-size: 24px; color: #5f6368;">menu</span>
                </button>

                <a class="navbar-brand me-0 px-md-4 fs-5 fw-bold d-flex align-items-center" href="<?= BASE_URL ?>/dashboard">
                    <span class="text-primary me-2 d-flex align-items-center">
                        <?php if (!empty($settings['company_logo'])): ?>
                            <img src="<?= BASE_URL ?>/<?= htmlspecialchars($settings['company_logo']) ?>" alt="Company Logo" class="img-fluid" style="max-height: 32px; border-radius: 4px;">
                        <?php else: ?>
                            <span class="material-symbols-outlined icon me-2" style="font-size: 28px;">storefront</span>
                        <?php endif; ?>
                    </span>
                    <span style="color: #444746; font-size: 22px; font-weight: 400; letter-spacing: -0.5px;"><?= e($settings['company_name'] ?? 'POS') ?></span>
                </a>
            </div>
            ...
            <div class="d-md-none d-flex align-items-center pe-3">
                <a href="<?= BASE_URL ?>/profile" class="d-block link-dark text-decoration-none" title="My Profile">
                     <?php if (!empty($_SESSION['profile_image'])): ?>
                         <img src="<?= BASE_URL ?>/<?= htmlspecialchars($_SESSION['profile_image']) ?>" class="rounded-circle border" style="width: 32px; height: 32px; object-fit: cover;">
                     <?php else: ?>
                         <div class="rounded-circle d-flex align-items-center justify-content-center" style="width: 32px; height: 32px; font-weight: 500; background-color: #0b57d0; color: white;">
        ...
        <!-- Search Bar -->
        <div class="mx-3 mx-md-auto d-flex justify-content-center align-items-center w-100 mt-2 mt-md-0 mb-2 mb-md-0 order-3 order-md-2" style="max-width: 720px;">
            <div class="google-search-container w-100 me-2">
                <span class="material-symbols-outlined google-search-icon">search</span>
                <input class="google-search-input" type="text" placeholder="Search items in POS..." aria-label="Search" id="globalSearch">
            </div>
            
            <div id="statusIndicators" class="d-flex gap-2 ms-2">
                <span id="internetStatus" class="badge rounded-pill bg-success d-flex align-items-center" title="Internet Status" style="padding: 6px 10px;">
                    <span class="material-symbols-outlined" style="font-size: 16px;">wifi</span>
                </span>
                <span id="dbStatus" class="badge rounded-pill bg-success d-flex align-items-center" title="Database Status" style="padding: 6px 10px;">
        ...
        <div class="navbar-nav nav-side-right flex-row align-items-center justify-content-end pe-4 d-none d-md-flex order-2 order-md-3">
             <div class="nav-item text-nowrap d-flex align-items-center">
                 <a href="<?= BASE_URL ?>/profile" class="d-block link-dark text-decoration-none" title="My Profile">
                     <?php if (!empty($_SESSION['profile_image'])): ?>
                         <img src="<?= BASE_URL ?>/<?= htmlspecialchars($_SESSION['profile_image']) ?>" class="rounded-circle border" style="width: 40px; height: 40px; object-fit: cover;">
                     <?php else: ?>
                         <div class="rounded-circle d-flex align-items-center justify-content-center" style="width: 40px; height: 40px; font-weight: 500; font-size: 18px; background-color: #0b57d0; color: white;">
                             <?= strtoupper(substr($_SESSION['username'] ?? 'U', 0, 1)) ?>
                         </div>
                     <?php endif; ?>
                 </a>
